...
func TestTime(t *testing.T) {
	...
	t.Run("Valid", func(t *testing.T) {
		...
		assert.NoError(t, err)
		assert.True(t, expected.F1.Equal(result.F1))
		assert.True(t, expected.F2.Equal(*result.F2))
		assert.True(t, expected.F3.Equal(result.F3))
	})
...
func TestUnexportedFields(t *testing.T) {
	...
	err = Parse(values, &st)
	assert.Error(t, err)
	assert.ErrorIs(t, err, ErrUnexportedStruct)
}
